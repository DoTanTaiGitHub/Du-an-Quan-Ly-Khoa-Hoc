...
public class HocVienDAO extends EduSysDao<HocVien, String> {
    ...
    public void insert(HocVien enity) {
        try {
            XJdbc.update(INSERT_SQL,
                    Integer.valueOf(enity.getMaKH()), enity.getMaNH(), Float.valueOf(enity.getDiem()));
    ...
    public void update(HocVien enity) {
        try {
            XJdbc.update(UPDATE_SQL,
                    Integer.valueOf(enity.getMaKH()), enity.getMaNH(), Float.valueOf(enity.getDiem()), Integer.valueOf(enity.getMaHV()));
        } catch (SQLException ex) {
            Logger.getLogger(HocVienDAO.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    public HocVien selectById(String id) {
        List<HocVien> list = this.selectBySQL(SELECT_BY_ID_SQL, Integer.valueOf(id));
        if (list.isEmpty()) {
            return null;
        }
        return list.get(0);
